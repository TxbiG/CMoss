...
static int ParseIPv4OrIPv6 ( const char** ppszText, uint8* abyAddr, uint16* pnPort, int* pbIsIPv6 )
{
...
int enet_address_get_host_ip(const ENetAddress * address, char * name, size_t nameLength) {
    ...
    else if (address->type == ENET_ADDRESS_TYPE_IPV6)
    {
        ...
        {
            if (address->port != 0) { if (nameLength < MS_IPv4MappedIPv6WithPort) { return -1; } sprintf(name, "[::ffff:%u.%u.%u.%u]:%u", (address->host.v6[6] >> 8) & 0xFF, (address->host.v6[6] >> 0) & 0xFF, (address->host.v6[7] >> 8) & 0xFF, (address->host.v6[7] >> 0) & 0xFF, address->port); }
            else { if (nameLength < MS_IPv4MappedIPv6) { return -1; } sprintf(name, "::ffff:%u.%u.%u.%u", (address->host.v6[6] >> 8) & 0xFF, (address->host.v6[6] >> 0) & 0xFF, (address->host.v6[7] >> 8) & 0xFF, (address->host.v6[7] >> 0) & 0xFF); }
        }
        else
        {
            /* standard IPv6 
            ...
            */
            ...
            for (i = 0; i < 8; ++i)
            {
                ...
                {
                    for (j = i + 1; j < 8; ++j) { if (address -> host.v6[j] != 0) { break; } }
                    if (f0 == -1 || j - i > l0 - f0) { f0 = i; l0 = j; }
                }
            }
...
int enet_address_set_host_ip(ENetAddress * address, const char * name) {
    ...
    if (!ParseIPv4OrIPv6(&name, result, &port, &isIPv6)) { return -1; }
...
void enet_address_convert_ipv6(ENetAddress * address)
{
    ...
    if (address->type == ENET_ADDRESS_TYPE_IPV4)
    {
        ...
        address->host.v6[6] = ((uint16) ipv4[0]) << 8 | ipv4[1];
        address->host.v6[7] = ((uint16) ipv4[2]) << 8 | ipv4[3];
    }
}