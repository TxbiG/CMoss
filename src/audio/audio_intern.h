...
typedef enum AudioFormat {
    ...
    AUDIO_FORMAT_S24LE   = 0x8020u,  // Add this
    AUDIO_FORMAT_S32LE   = 0x8030u,  // 32-bit integer samples.
    AUDIO_FORMAT_S32BE   = 0x9030u,  // As above, but big-endian byte order.
    AUDIO_FORMAT_F32LE   = 0x8120u,  // 32-bit floating point samples.
    AUDIO_FORMAT_F32BE   = 0x9120u,  // As above, but big-endian byte order.
    AUDIO_FORMAT_F64LE   = 0x8140u,  // 64-bit float (double)
    AUDIO_FORMAT_F64BE   = 0x9140u
};
...
struct Moss_RayAudioListener2D {
    Moss_AudioListener2D* base;   // Reference to the normal 2D listener
...
struct Moss_RayAudioListener3D {
    Moss_AudioListener3D* base;   // Reference to the normal 3D listener
...
void Audio_MixChannel(ChannelID id, float* buffer, uint32_t frames, uint32_t channels) {
    ...
    // 1. Effects
    for (AudioEffect* fx = ch->effects; fx; fx = fx->next)
...
static uint32_t ReadU32LE(const uint8_t* data) { return data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24); }
static uint16_t ReadU16LE(const uint8_t* data) { return data[0] | (data[1] << 8); }
...
float DopplerPitch(const Vec3& listenerPos, const Vec3& listenerVel,const Vec3& sourcePos,const Vec3& sourceVel,float speedOfSound = 343.0f, float dopplerFactor = 1.0f) {
    ...
    return clamp(pitch, 0.5f, 2.0f); // typical safe audio range
}
